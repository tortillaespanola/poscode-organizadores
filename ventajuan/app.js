/* =========================================================
   VENTA · JUAN — punto de venta independiente, con URL propia
   (no aparece en el selector de /venta/ ni en data/puntos_venta.json).
   ========================================================= */
...
// Punto de venta fijo: esta app no tiene selector, solo existe "Juan".
const PUNTO_ACTIVO = "juan";

// Catálogo propio de Juan, no el compartido data/articulos.json.
const RUTA_ARTICULOS = "../data/articulos_juan.json";

let ticket = [];              // [{nombre, precio, cantidad}]
let cacheVentas = null;       // {sha, data:[...]}
let cacheCierres = null;      // {sha, data:[...]}
let catalogoArticulos = [];   // [{nombre, precio}], de data/articulos_juan.json

/* ---------------- Catálogo ---------------- */

async function cargarDatosEstaticos() {
  const res = await fetch(RUTA_ARTICULOS, { cache: "no-store" });
  catalogoArticulos = await res.json();
}
...
async function asegurarCacheVentas() {
  if (!cacheVentas) cacheVentas = await githubGetFile(rutaVentas(PUNTO_ACTIVO));
  return cacheVentas;
}
...
async function refrescarHistorial() {
  ...
  setCargando(true);
  try {
    cacheVentas = await githubGetFile(rutaVentas(PUNTO_ACTIVO));
...
(async function iniciar() {
  try {
    ...
    console.error(e);
    mostrarToast("No se pudo cargar el catálogo de artículos.", true);
  }
