/* =========================================================
   CAJA · ORGANIZADORES — cierre consolidado de todos los
   puntos de venta. No enlazada desde /venta/.
   ========================================================= */
...
// Ruta de datos estáticos: relativa a esta página (/caja/), no a la raíz
// del repo como las rutas de la API de GitHub.
...
async function cargarPuntosVenta() {
  const res = await fetch(RUTA_PUNTOS_VENTA, { cache: "no-store" });
  puntosVenta = await res.json();
}
...
(async function iniciar() {
  try {
    ...
    console.error(e);
    mostrarToast("No se pudo cargar la lista de puntos de venta.", true);
  }
